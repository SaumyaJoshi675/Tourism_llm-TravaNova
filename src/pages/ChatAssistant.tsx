...
import { Send, Image as ImageIcon, Mic, Sparkles, BrainCircuit, MapPin, Download } from 'lucide-react';
...
export default function ChatAssistant() {
  ...
  const handleSend = async () => {
    ...
    setInput('');
    setIsTyping(true);
    // setStatus('generating'); // Let's skip 'searching' for now as we are relying on backend "chat"
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/chat', {
        ...
        body: JSON.stringify({
          session_id: 'user-session-1', // Simple session for now
          message: userQuery
        })
      });
      ...
      while (true) {
        ...
        // Check if it's the JSON fallback (non-stream)
        if (fullText.length === 0 && chunk.trim().startsWith('{')) {
          try {
            const json = JSON.parse(chunk);
            if (json.reply) {
              fullText = json.reply;
              isJsonReply = true;
              // It was a simple reply (not a stream)
            }
          } catch (e) {
  ...
  const downloadItinerary = (content: string) => {
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `TravaNova_Itinerary_${new Date().toISOString().slice(0, 10)}.txt`;
